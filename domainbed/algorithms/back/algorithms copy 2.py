...
class iDAG(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.network = nn.Sequential(self.featurizer, self.encoder, self.dag_module, self.classifier)
    ...
    def update(self, x, y, **kwargs):
        ...
        add_loss = torch.tensor(0.0, device=all_x.device)
        loss_interv = torch.tensor(0.0, device=all_x.device)
        if kwargs["step"] > 1*self.size_dataloader:
            self.update_probabilities(logit, dom_logit, all_y, all_dom)
            self.update_prototypes(feat, all_y)
        if kwargs["step"] > 400:    
            f_domain = self.dag_module.forward_reverse(feat) # select domain feature
            logit_ = self.domain_clf(f_domain.detach())
            add_loss += self._cross_entropy(logit_, all_dom)
            f_class = self.dag_module(feat) # select class feature
            logit_ = self.classifier(f_class)
            add_loss += self._cross_entropy(logit_, all_y)
        if kwargs["step"] > 400:
            ...
            intervention = self.dag_module.forward_reverse(feat_recon) # select domain feature
            logit_interv = self.classifier((intervention + feat)*0.5)
            loss_interv += self._cross_entropy(logit_interv, all_y)
    
        loss = loss + reg_loss + add_loss + 0.1*loss_interv
        ...
        return {"loss": loss.item(), "add_loss": add_loss.item(), "reg_loss": reg_loss.item(), "loss_interv":loss_interv.item(), "dag_loss": dag_loss.item()}

    def predict(self, x):
        feat = self.encoder(self.featurizer(x))
        f_masked = self.dag_module(feat)
        logit = self.classifier(f_masked)
        return logit
