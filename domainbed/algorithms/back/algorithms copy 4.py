...
class NotearsMLP(nn.Module):
    ...
    def project(self):
        # 1) 비음수 제약
        self.fc1_pos.weight.clamp_(min=0.0)
        self.fc1_neg.weight.clamp_(min=0.0)

        # 2) 마스킹 (mask가 {0,1} 또는 비음수라고 가정)
        d, m1 = self.dims[0], self.dims[1]
        self.fc1_pos.weight.view(d, m1, d).mul_(self.projection_mask)
        self.fc1_neg.weight.view(d, m1, d).mul_(self.projection_mask)
...
class iDAG(Algorithm):
    ...
    def update(self, x, y, **kwargs):
        ...
        if kwargs["step"] > 2*self.size_dataloader:
            input_dag = torch.cat([feat, logit, dom_logit], dim=-1).detach() # logit and dom_logit are dummy data
            out_recon =self.dag_module.forward_(input_dag)
            feat_recon = out_recon[:,:feat.shape[-1]]  # (B, D)
            z_causal  = self.dag_module(feat, feat_recon) # causal part 강조, domain에 중요한 부분에 recon을 더했을 때 classification 성능은 유지되어야 함
            ...
            loss_interv += 0.01*self._cross_entropy(logit_spurious, all_dom)
    
        loss = loss + reg_loss + loss_interv
