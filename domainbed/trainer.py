...
def train(test_envs, args, hparams, n_steps, checkpoint_freq, logger, writer, target_env=None):
    ...
    rho_freq = int(4*steps_per_epoch)
    for step in range(n_steps):
        # rho update
        if step % rho_freq == 0:
            algorithm.eval()
            dists = []
            for _ in range(rho_freq//4):
                ...
                with torch.no_grad():
                    zz, yy = algorithm.get_emb(**inputs) 
                d2 = algorithm.recompute_rho_adv_ref(zz, yy)
                dists.append(d2)
            all_d2 = np.concatenate(dists, axis=0)
            all_d = np.sqrt(all_d2 + 1e-12)
            rho = float(all_d.mean())
            if math.isfinite(rho):
                def clamp(value, min_value, max_value):
                    return max(min_value, min(value, max_value))
                rho = clamp(rho,1e-2,1e2)
        ...
        if step and step % checkpoint_freq == 0:
            results = {
                "step": step,
